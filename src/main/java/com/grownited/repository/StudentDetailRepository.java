...
public interface StudentDetailRepository extends JpaRepository<StudentDetailEntity, Integer>{
	...
	@Query(value = "SELECT sd.*,concat(u.first_name, ' ', u.last_name) as FullName, u.email, cl.college_name AS CollegeName FROM  student_detail sd JOIN college cl ON sd.college_id = cl.college_id join users u on sd.user_id = u.user_id and sd.student_detail_id = :studentDetailId",nativeQuery = true)
