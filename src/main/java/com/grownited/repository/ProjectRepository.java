...
public interface ProjectRepository extends JpaRepository<ProjectsEntity, Integer>{
	...
	//for admin list get all company and users details.
	@Query(value = "select p.*, cmp.company_name, cmp.address, cmp.person_name, concat(u.first_name, ' ',u.last_name) as FullName, u.role from projects p join company cmp on cmp.company_id = p.company_id join users u on u.user_id = cmp.user_id", nativeQuery = true)
	List<Object[]> GetAll();
	
	//for admin view get all company and users details By Id.
	...
	//ajax findByCompanyId so we get the projectname:
	List<ProjectsEntity> findByCompanyId(Integer companyId);
